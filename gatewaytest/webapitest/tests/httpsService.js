...
describe('HTTPS TestService API Tests', () => {
  ...
  const httpsRoutePath = '/testbasic-https';
  ...
  before(() => {
    ...
    return KongManager.createService('https://mockserver:1080', { name: httpsTestService }).then((id) => {
      ...
      return KongManager.createRoute(id, { name: httpsRouteName, path: httpsRoutePath, methods: ['GET','POST','PUT','DELETE','PATCH','OPTIONS','HEAD'] }).then((routeId) => {
